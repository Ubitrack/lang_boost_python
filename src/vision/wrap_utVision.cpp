...
struct image_to_ndarray {
	static PyObject* convert(Vision::Image const& p) {
		const Mat m(p);
...
struct image_to_python_converter {
	image_to_python_converter& to_python() {
		bp::to_python_converter<Vision::Image,
				image_to_ndarray, false //image_to_array has no get_pytype
		>();
		return *this;
	}
};

static PyObject* get_image_as_array(bp::object const & self) {
	const Mat m(bp::extract<Measurement::ImageMeasurement const &>(self)().get());
	return pyopencv_from(m);
}
